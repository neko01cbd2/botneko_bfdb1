var 
  ...
  /*
  getTikTok = async function (url, path) {
    var
      res = function () {},
      rej = function () {},
      rePro = new Promise(function (resolve, reject) {
    ...
    await axios({
      method: 'GET',
      url: 'https://docs-api.nttvn.repl.co/tiktok/download?link=' + url
    })
      .then(async function ({ 
        data: { 
          play_count,
        ...
        fs.writeFileSync(path, Buffer.from(buffer, 'utf-8'));
        res(
          `→ 𝗧𝗮́𝗰 𝗴𝗶𝗮̉: ${author.nickname}\n→ 𝗨𝗜𝗗: ${author.unique_id}\n━━━━━━━━━━━━━━━━━━\n→ 𝗞𝗵𝘂 𝘃𝘂̛̣𝗰: ${region}\n→ 𝗡𝗼̣̂𝗶 𝗱𝘂𝗻𝗴: ${title}\n→ 𝗟𝘂̛𝗼̛̣𝘁 𝘅𝗲𝗺: ${play_count}\n→ 𝗦𝗼̂́ 𝘁𝗶𝗺: ${digg_count}\n→ 𝗕𝗶̀𝗻𝗵 𝗹𝘂𝗮̣̂𝗻: ${comment_count}\n→ 𝗧𝗮̉𝗶 𝘅𝘂𝗼̂́𝗻𝗴: ${download_count}\n→ 𝗧𝗵𝗼̛̀𝗶 𝗴𝗶𝗮𝗻: ${duration} giây\n━━━━━━━━━━━━━━━━━━\n→ 𝗔̂𝗺 𝗻𝗵𝗮̣𝗰: ${music_info.title}\n→ 𝗧𝗵𝗼̛̀𝗶 𝗴𝗶𝗮𝗻: ${music_info.duration} giây`
        );
      })
      .catch(rej);
    return rePro;
  ...
  handleEvent = async function ({ api, event, Threads }) {
    ...
    for (i of event.body.split(" ")) {
      if (i.indexOf("https://") == 0) {
        for (e of i.split("/")) {
          /*if (e == "vt.tiktok.com" || e == "www.tiktok.com") {
            try {
              var q = await getTikTok(i, path);
              api.sendMessage({
                body: q,
                attachment: fs.createReadStream(path)
              }, event.threadID, () => fs.unlinkSync(path));
            } catch (e) {
              api.sendMessage(e.message, event.threadID);
            }
          }*/
